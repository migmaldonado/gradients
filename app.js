...
//declares a folder for static content that is publically accessible
app.use(express.static('public'));
app.use(layouts);

app.set('layout','layouts/main-layout.ejs');

app.set('view engine','ejs'); //remember we installed npm ejs

//a dynamic website produces different responses (content)
...
const citiesList= ['Vail','Telluride','Durango'];
const foodList= ['beer','melons','tacos','apples','cider','rum'];

app.get('/cities',(req,res,next)=>{
  res.render('cities-view.ejs',{cities:citiesList, name:'Suqui', age:30,foods:foodList});
});
...
app.get('/colors',(req,res,next)=>{
  res.render('colors-view.ejs',{mycolors:colors});
});

app.get('/pink',(req,res,next)=>{
  res.render('pink-view.ejs',{mycolors:colors});
});
app.get('/purple',(req,res,next)=>{
  res.render('purple-view.ejs',{mycolors:colors});
});
app.get('/blue',(req,res,next)=>{
  res.render('blue-view.ejs',{mycolors:colors});
});
app.get('/random',(req,res,next)=>{
  res.render('random-view.ejs',{mycolors:colors});
});
